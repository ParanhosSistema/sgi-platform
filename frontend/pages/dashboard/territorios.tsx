...
export default function Territorios() {
  ...
  const loadData = async (token: string) => {
    try {
      ...
      const res = await fetch(`${API_URL}/territorios`, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const API_URL = process.env.NEXT_PUBLIC_API_URL || '';
    const url = editingId ? `${API_URL}/territorios/${editingId}` : `${API_URL}/territorios`;
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert('Erro ao salvar território');
      }
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Deseja realmente excluir este território?')) return;
    ...
    try {
      const res = await fetch(`${API_URL}/territorios/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div style={styles.container}>
      <header style={styles.header}>
        <h1 style={styles.title}>SGI Platform - Territórios</h1>
      ...
      <nav style={styles.nav}>
        ...
        <Link href="/dashboard/territorios" style={{ ...styles.navLink, backgroundColor: '#e0e7ff' }}>
          🗺️ Territórios
        </Link>
      ...
      <main style={styles.main}>
        <div style={styles.pageHeader}>
          <h2 style={styles.subtitle}>Territórios</h2>
          <button 
            onClick={() => {
              setShowForm(true);
              setEditingId(null);
                ...
                {territorios.length === 0 ? (
                  ...
                  territorios.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td style={styles.td}>
                        <button onClick={() => handleEdit(t)} style={styles.editBtn}>Editar</button>
